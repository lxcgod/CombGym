...
class VariationalAutoencoder:
    ...
    def __init__(self,
        ...
        self.global_scale = theano.shared(np.float32(global_scale))
        ...
        create_weight = lambda dim_input, dim_output: np.random.normal(0.0, np.sqrt(2.0 / (dim_input + dim_output)), (dim_input, dim_output)).astype(theano.config.floatX)
        create_weight_zeros = lambda dim_input, dim_output: np.zeros((int(dim_input), int(dim_output))).astype(theano.config.floatX)
        create_bias = lambda dim_output: 0.1 * np.ones(dim_output, dtype=theano.config.floatX)


        # Variational uncertainty
        create_weight_logsig = lambda dim_input, dim_output: self.logsig_init \
            * np.ones((int(dim_input), int(dim_output))).astype(theano.config.floatX)
    ...
    def save_parameters(self, file_prefix):
        ...
        cPickle.dump({name: p.get_value() for name, p in self.params.items()},\
            open("/thinker/glusterfs/home/TSLab-xcLu/DeepSequence-master/DeepSequence-master/examples/params/"+"mTagBFP2_b0.5" + "_params.pkl", "wb")) 
        cPickle.dump({name: m.get_value() for name, m in self.m.items()}, \
            open("/thinker/glusterfs/home/TSLab-xcLu/DeepSequence-master/DeepSequence-master/examples/params/"+"mTagBFP2_b0.5" +"_m.pkl", "wb")) 
        cPickle.dump({name: v.get_value() for name, v in self.v.items()}, \
            open("/thinker/glusterfs/home/TSLab-xcLu/DeepSequence-master/DeepSequence-master/examples/params/"+"mTagBFP2_b0.5" +"_v.pkl", "wb")) 

    def load_parameters(self, file_prefix=""):
        """Load the variables in a shared variable safe way"""
        p_list = cPickle.load(open("/thinker/glusterfs/home/TSLab-xcLu/DeepSequence-master/DeepSequence-master/examples/params/"+file_prefix \
            + "_params.pkl", "rb"))
        m_list = cPickle.load(open("/thinker/glusterfs/home/TSLab-xcLu/DeepSequence-master/DeepSequence-master/examples/params/"+file_prefix \
            + "_m.pkl", "rb"))
        v_list = cPickle.load(open("/thinker/glusterfs/home/TSLab-xcLu/DeepSequence-master/DeepSequence-master/examples/params/"+file_prefix \
            + "_v.pkl", "rb"))
